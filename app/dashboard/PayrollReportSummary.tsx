  ...
  ({ reports, employeeList }) => {
    ...
    const payrollData = useMemo(() => {
      ...
      const isManagerGroup = (groupName: string) => {
        ...
        return (
          lowerGroup.includes("manager") || lowerGroup.includes("management")
        );
      };
      ...
      const individualResults = individualEmployees.map((employee) => {
        ...
        return {
          ...
          monthlyRange,
          isSpecialIndividual:
            employee.department !== "ITH-OE" && !!employee.group, // Flag to identify special individuals for UI if needed
      ...
      // Build groups from both static list AND actual reports
      ...
      // Ensure employees who reported under a specific group are included in that group,
      // even if their static list doesn't say so (e.g. temporary assignment or data mismatch)
      ...
      const departmentResults = Object.keys(reportBasedGroups)
        .sort()
        .map((deptName) => {
          const deptGroups = reportBasedGroups[deptName];

          const calculatedSubGroups = Object.keys(deptGroups).map(
            (groupName) => {
                ...
                <div className="mb-4 space-y-3">
                  ...
                  <div className="flex gap-2 flex-wrap items-center">
                    ...
                    <Button
                      size="sm"
                      variant={
                        statusFilter === "eligible" ? "default" : "outline"
                      }
                    ...
                    >
                      💰 ได้รับ (
                      {
                        payrollData.individuals.filter((e) => e.isEligible)
                          .length
                      }
                      )
                    </Button>
                      ...
                      {paginatedIndividuals.map((employee, index) => (
                        ...
                        >
                          ...
                          <td className="p-2 font-medium">
                            <button
                              onClick={() =>
                                handleViewEmployeeReports(employee)
                              }
            ...
            <AccordionItem value="other-depts" className="border-none">
              ...
              <AccordionContent className="p-4 bg-gray-50">
                ...
                <div className="space-y-6">
                      ...
                      >
                        <div className="px-4 py-3 bg-gray-100 border-b flex justify-between items-center">
                          ...
                          <Badge variant="outline" className="bg-white">
                            {dept.totalEligibleSubGroups} /{" "}
                            {dept.totalSubGroups} กลุ่มผ่านเกณฑ์
                          </Badge>
                        </div>
                        <div className="p-2">
                          <Accordion
                                  ...
                                  <div className="flex flex-1 items-center justify-between mr-2">
                                    <div className="text-left flex items-center gap-3">
                                      ...
                                      <span className="text-xs text-gray-500">
                                        (สมาชิก {group.totalEmployees} คน |
                                        ส่งรวม {group.totalBbsCount}/12)
                                      </span>
                                    </div>
                                  ...
                                  <div className="mt-2 text-xs text-gray-600 mb-2 pl-1">
                                    {group.isEligible ? (
                                      <span className="text-green-600">
                                        ✓ ยอดส่งรวม {group.totalBbsCount} ครั้ง
                                        (เป้า 12) | ไม่มีการรายงานจาก SHE
                                        เกินเกณฑ์
                                      </span>
                                    ) : (
                  ...
                  {groupSearchQuery &&
                    payrollData.departments.every((dept) => {
                      const filteredSubGroups = dept.subGroups.filter((group) =>
